...
describe('UseCaseValidatableAdapter', () => {
  describe('validate', () => {
    test('Quando MockAdapter é válido, espera-se que ele não lance Exception', async () => {
      const validInstance: MockAdapter = new MockAdapter('42');
      await expect(validInstance.validate()).resolves.toBeUndefined();
    });

    test('Quando MockAdapter não é válido, espera-se que ele lance Exception', async () => {
